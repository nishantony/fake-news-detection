...
train_label$final_label[train_label$final_label=='unrelated'] <- 'not fake'
...
not_fake_cloud <- which(train_label_final$final_label=='not fake')
...
set.seed(7)  
df <- sample(1:2,size=length(train_label_final$final_label),replace=TRUE,prob=c(0.6,0.4))
train <- train_label_final[df==1,]
verification <- train_label_final[df==2,]


#separating the labels for each train and verification parts.

train_label <- train_label$final_label
verification_label <- verification$final_label

#just checking the proportion of the data
prop.table(table(train_label))
prop.table(table(verification_label))

#######separating the data of the train part into its labels (fake/true) ##########
train_fake <- subset(train_label,final_label == 'fake')
train_not_fake <- subset(train_label,final_label == 'not fake')

#######separating the data of the verification part into its labels (fake/true) ##########
train_fake <- subset(verification_label,final_label == 'fake')
train_not_fake <- subset(verification_label,final_label == 'not fake')


#creating a clean corpus of 'news'
...
#creating a DTM of 'news' from our clean corpus
news_dtm <- DocumentTermMatrix(news_corpus_clean)

#creating a clean corpus of 'labels'
news_corpus <- Corpus(VectorSource(train_label_final$final_label))
news_corpus_clean <- tm_map(news_corpus, tolower)
news_corpus_clean <- tm_map(news_corpus_clean, removeNumbers)
news_corpus_clean <- tm_map(news_corpus_clean, removePunctuation)
news_corpus_clean <- tm_map(news_corpus_clean, removeWords,stopwords())
...
#creating a DTM of 'labels' from our clean corpus
news_dtm <- DocumentTermMatrix(news_corpus_clean)
...
'''
NEED TO CHECK WHICH PART WILL ENTER AND WHAT VARIABLE WILL USE!!!!
'''
