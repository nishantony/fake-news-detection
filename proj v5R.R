...
set.seed(7)  
#df <- sample(1:3,size=length(train_label_final$final_label),replace=TRUE,prob=c(0.6,0.2,0.2))
#train <- train_label_final[df==1,]
#verification <- train_label_final[df==2,]
#final_test<- train_label_final[df==3,]
sample = sample.split(train_label_final$final_label,SplitRatio = 0.6) # splits the data in the ratio mentioned in SplitRatio. After splitting marks these rows as logical TRUE and the the remaining are marked as logical FALSE
